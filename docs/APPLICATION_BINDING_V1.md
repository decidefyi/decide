...
The required material is response material generated by Decide. A Krafthaus
application must not send `runtime_binding`, `trusted_adapter`, `adapter_facts`,
`rulebook_attestation`, or verdict/action fields as request authority. Rulebook
requests that attempt to preload those fields fail closed with
`RULEBOOK_OUTPUT_MATERIAL_FORBIDDEN`, including when the fields are nested inside
`context.inputs` or emitted by adapter facts.
