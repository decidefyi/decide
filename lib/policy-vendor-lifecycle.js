...
export function buildPolicyVendorLifecycleReport({
  ...
  const candidates = Object.entries(candidateRegistry?.candidates || {})
    .map(([vendor, config]) => {
      ...
      const admittedPolicies = Object.keys(config?.admitted_policies || {}).sort();
      return {
        vendor,
        display_name: String(config?.display_name || vendor),
        lifecycle: admittedPolicies.length ? "partially_admitted" : (ready ? "ready_for_review" : "candidate"),
        admitted_policies: admittedPolicies,
