...
function krafthausWorkflowReadinessV1(input) {
  const evidenceSources = boundedInteger(input.evidence_sources, 0, 12);
  const integrationTouchpoints = boundedInteger(input.integration_touchpoints, 0, 12);
  const exceptionPaths = boundedInteger(input.exception_paths, 0, 20);
  const ownerPresent = input.owner_present === true;
  const automationBoundaryDefined = input.automation_boundary_defined === true;
  const rollbackPathDefined = input.rollback_path_defined === true;
  const recurringWorkflow = ["daily", "weekly"].includes(input.repeat_frequency);
  const actionRisk = input.action_risk;
  const valueAtStake = input.value_at_stake;
  const handoffRisk =
    actionRisk === "high" || exceptionPaths >= 4 || integrationTouchpoints >= 4
      ? "high"
      : actionRisk === "medium" || exceptionPaths >= 2 || integrationTouchpoints >= 2
        ? "medium"
        : "low";
  const riskPenalty = { low: 0, medium: 8, high: 22 }[actionRisk];
  const valueBonus = { low: 2, medium: 5, high: 10 }[valueAtStake];
  const workflowScore = Math.max(
    0,
    Math.min(
      100,
      (ownerPresent ? 18 : 0) +
        (recurringWorkflow ? 12 : 0) +
        Math.min(evidenceSources, 4) * 5 +
        (automationBoundaryDefined ? 18 : 0) +
        (rollbackPathDefined ? 14 : 0) +
        valueBonus -
        riskPenalty -
        Math.max(0, integrationTouchpoints - 1) * 5 -
        exceptionPaths * 4
    )
  );
  const readyToBind =
    ...
    recurringWorkflow &&
    evidenceSources >= 2 &&
    automationBoundaryDefined &&
    rollbackPathDefined &&
    handoffRisk !== "high" &&
    workflowScore >= 78;
  const workflowBand = !ownerPresent || evidenceSources < 1 || handoffRisk === "high" ? "blocked" : readyToBind ? "bind_ready" : "route_review";
  ...
  return {
    ...
    automation_boundary_defined: automationBoundaryDefined,
    rollback_path_defined: rollbackPathDefined,
    exception_paths: exceptionPaths,
    action_risk: actionRisk,
    value_at_stake: valueAtStake,
    workflow_score: workflowScore,
    workflow_band: workflowBand,
    handoff_risk: handoffRisk,
    ready_to_bind: readyToBind,
  };
}
...
const KRAFTHAUS_WORKFLOW_READINESS_V1_MANIFEST = Object.freeze({
  manifest_version: TRUSTED_ADAPTER_MANIFEST_VERSION,
  adapter_id: "krafthaus_workflow_readiness",
  version: "1.0.0",
  implementation_revision: "krafthaus_workflow_readiness_score_v1",
  description: "Normalizes bounded Krafthaus workflow application readiness facts before declarative rulebook evaluation.",
  input_schema: {
    required: [
      "owner_present",
      "repeat_frequency",
    ...
    properties: {
      owner_present: { type: "boolean" },
      repeat_frequency: { type: "string", enum: ["rare", "weekly", "daily"] },
      evidence_sources: { type: "number", maximum: 1000 },
      integration_touchpoints: { type: "number", maximum: 1000 },
      automation_boundary_defined: { type: "boolean" },
      rollback_path_defined: { type: "boolean" },
      exception_paths: { type: "number", maximum: 1000 },
      action_risk: { type: "string", enum: ["low", "medium", "high"] },
      value_at_stake: { type: "string", enum: ["low", "medium", "high"] },
    },
    additional_properties: false,
  },
  output_schema: {
    properties: {
      owner_present: { type: "boolean" },
      repeat_frequency: { type: "string", enum: ["rare", "weekly", "daily"] },
      recurring_workflow: { type: "boolean" },
      evidence_sources: { type: "integer", minimum: 0, maximum: 12 },
      integration_touchpoints: { type: "integer", minimum: 0, maximum: 12 },
      automation_boundary_defined: { type: "boolean" },
      rollback_path_defined: { type: "boolean" },
      exception_paths: { type: "integer", minimum: 0, maximum: 20 },
      action_risk: { type: "string", enum: ["low", "medium", "high"] },
      value_at_stake: { type: "string", enum: ["low", "medium", "high"] },
      workflow_score: { type: "integer", minimum: 0, maximum: 100 },
      workflow_band: { type: "string", enum: ["blocked", "route_review", "bind_ready"] },
      handoff_risk: { type: "string", enum: ["low", "medium", "high"] },
      ready_to_bind: { type: "boolean" },
    },
    additional_properties: false,
  },
  capabilities: {
...
const REGISTERED_ADAPTERS = new Map([
  ...
  [
    `${KRAFTHAUS_WORKFLOW_READINESS_V1_MANIFEST.adapter_id}@${KRAFTHAUS_WORKFLOW_READINESS_V1_MANIFEST.version}`,
    {
      manifest: KRAFTHAUS_WORKFLOW_READINESS_V1_MANIFEST,
      execute: krafthausWorkflowReadinessV1,
    },
  ],
]);
...
const TRUSTED_ADAPTER_VERSION_LOCKS = Object.freeze({
  ...
  "krafthaus_workflow_readiness@1.0.0": Object.freeze({
    adapter_id: "krafthaus_workflow_readiness",
    version: "1.0.0",
    implementation_hash: "37afe50da879b918c3ca7a558a5c066b8d9025b73685e7c0c6d916de66092440",
