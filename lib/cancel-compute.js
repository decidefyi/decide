...
function withSource(result, vendor) {
  ...
  return withLineage({
    ...
    policy_last_verified_utc: source?.last_verified_utc || policySources.last_verified_utc || null,
 ...
 */
export function compute(request, { requireCompleteContext = true, evidenceSnapshot = null, now = new Date() } = {}) {
  let { vendor, region, plan, billing_cadence } = request;
  ...
  // Scoped admissions accept only the published request facts. In particular,
  // caller-supplied evidence, clocks or output flags cannot widen the scope.
  if (v?.required_scope && Object.keys(request).some(field =>
    !Object.hasOwn(bindPolicyRequest("cancel", request).parameters, field))) {
    return withSource({ verdict: "UNKNOWN", code: "INVALID_REQUEST_CONTEXT",
      message: "Use only the published cancellation request fields. Evidence, clocks and execution outputs are server-owned.",
      rules_version: rules.rules_version, vendor, automation_safe: false }, vendor);
  }
  ...
  const scope = v?.required_scope || {};
  const missingScope = Object.entries(scope).filter(([field, expected]) => request[field] !== expected).map(([field]) => field);
  const scoped = Object.keys(scope).length > 0;
  const contextComplete = missingScope.length > 0 || requiresManualReview
    ? false
    : ((!requireCompleteContext && !scoped) || !requiresBillingCadence || Boolean(billing_cadence));
  ...
  const rulebookResult = evaluateCancelPolicyRulebook({
    policy_request: bindPolicyRequest("cancel", { ...request, vendor, region, plan, billing_cadence }),
    policy_evidence_current: policyEvidence.current,
    policy_evidence: policyEvidence,
    region_supported: region === "US",
    plan_supported: plan === (v?.supported_plan || "individual"),
    vendor_supported: Boolean(v),
    context_complete: contextComplete,
    missing_context: contextComplete ? "" : (missingScope.length ? missingScope.join(",") : (requiresManualReview ? "manual_policy_review" : "billing_cadence")),
  ...
  if (rulebookResult.reason_code === "NON_INDIVIDUAL_PLAN") {
    return withRulebook({
      verdict: "UNKNOWN",
      code: rulebookResult.reason_code,
      message: `Plan "${plan}" is not supported for ${vendor}. Expected "${v?.supported_plan || "individual"}".`,
      rules_version: rules.rules_version,
    }, vendor, rulebookResult);
  }
  ...
  if (rulebookResult.reason_code === "MISSING_REQUIRED_CONTEXT") {
    const requiredContext = [...missingScope, ...(requiresManualReview ? ["manual_policy_review"] : (!billing_cadence && requiresBillingCadence ? ["billing_cadence"] : []))];
    return withRulebook({
      verdict: "UNKNOWN",
      code: rulebookResult.reason_code,
      message: scoped
        ? `${vendor} requires an explicit supported product, purchase channel, contract, action and billing cadence. Supported scope: ${JSON.stringify(scope)}. No cancellation has been performed.`
  ...
  if (rulebookResult.reason_code === "PAID_TERM_END_CANCELLATION") {
    return withRulebook({
      ...
      billing_cadence,
      cancellation_effective: "paid_term_end",
      execution_performed: false,
      cancellation_confirmed: false,
      refund: "not_evaluated",
