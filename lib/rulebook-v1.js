...
export function evaluateRulebookV1({ rulebook, inputs, bindingMode } = {}) {
  ...
  if (!isRulebookBindingModeSupported(normalizedBindingMode)) {
    return {
      ok: false,
      statusCode: 422,
      error: "RULEBOOK_BINDING_MODE_UNSUPPORTED",
      message: "Rulebook v1 supports only declared declarative binding modes.",
      binding_mode: normalizedBindingMode,
      supported_binding_modes: [...RULEBOOK_SUPPORTED_BINDING_MODES],
    };
  }

  const normalizedInputs = isPlainObject(inputs) ? inputs : {};
  const callerSuppliedOutputFields = findRulebookOutputMaterialPaths(normalizedInputs, { path: "context.inputs" });
  if (callerSuppliedOutputFields.length) {
    return {
      ok: false,
      statusCode: 422,
      error: "RULEBOOK_OUTPUT_MATERIAL_FORBIDDEN",
      message: "Rulebook input facts cannot supply Decide-generated Decision Record material.",
  ...
  const runtimeBinding = buildRulebookRuntimeBinding({ bindingMode: normalizedBindingMode });
