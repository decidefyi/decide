...
function adoptionTrendPeriod(events, window, generatedAt) {
  ...
  return {
    ...
    internal_probes: report.internal_probes,
  };
}
...
function adoptionTrendWindowObserved(period) {
  return Number(period?.totals?.events || 0) > 0 || Number(period?.internal_probes?.events || 0) > 0;
}

function adoptionTrendComparison(current, previous) {
  const currentWindowObserved = adoptionTrendWindowObserved(current);
  const previousWindowObserved = adoptionTrendWindowObserved(previous);
  let reasonCode = "COMPLETE_WINDOWS";
  if (!currentWindowObserved && !previousWindowObserved) reasonCode = "BOTH_WINDOWS_UNOBSERVED";
  else if (!previousWindowObserved) reasonCode = "PRIOR_WINDOW_UNOBSERVED";
  else if (!currentWindowObserved) reasonCode = "CURRENT_WINDOW_UNOBSERVED";

  return {
    status: currentWindowObserved && previousWindowObserved ? "available" : "baseline_pending",
    current_window_observed: currentWindowObserved,
    previous_window_observed: previousWindowObserved,
...
export function buildMcpAdoptionTrend({
  ...
  return {
    ...
    current,
    previous,
    comparison: adoptionTrendComparison(current, previous),
    deltas: {
