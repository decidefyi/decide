...
function normalizeRuntimeBinding(binding) {
  ...
  const normalized = {
    production_core: String(binding.production_core || ""),
    binding_mode: String(binding.binding_mode || ""),
    verdict_authority: String(binding.verdict_authority || ""),
  };
  if (binding.adapter_authority !== undefined) {
    normalized.adapter_authority = String(binding.adapter_authority || "");
  }
  normalized.customer_supplied_code = String(binding.customer_supplied_code || "");
  return normalized;
}
...
export function buildRulebookAttestation(result = {}) {
  const bundle = {
    ...
    runtime_binding: normalizeRuntimeBinding(result.runtime_binding),
