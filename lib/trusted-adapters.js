...
function publicManifest(entry) {
  ...
  assertTrustedAdapterVersionLock(visibleManifest);
  return visibleManifest;
}

function assertTrustedAdapterVersionLock(manifest) {
  const lock = getTrustedAdapterVersionLock(manifest?.adapter_id, manifest?.version);
  if (!lock) return;

  const mismatches = [];
  if (manifest.implementation_hash !== lock.implementation_hash) mismatches.push("implementation_hash");
  if (manifest.manifest_hash !== lock.manifest_hash) mismatches.push("manifest_hash");
  if (!mismatches.length) return;

  const error = new Error(
    `Trusted adapter version lock drift for ${manifest.adapter_id}@${manifest.version}: ${mismatches.join(", ")}`
  );
  error.code = "TRUSTED_ADAPTER_VERSION_LOCK_DRIFT";
...
export async function executeTrustedAdapter(invocation) {
  ...
  try {
    ...
    return {
      ok: false,
      statusCode: 500,
      error: error?.code || "TRUSTED_ADAPTER_VERSION_LOCK_DRIFT",
      message: error?.message || "Trusted adapter version lock drifted.",
      mismatches: error?.mismatches,
      expected_lock: error?.expected,
      actual_manifest: error?.actual,
    };
  }
  ...
  if (!execution.ok) {
    return {
      ...
      denied_capabilities: execution.denied_capabilities,
    };
  }
