...
export function compute({ vendor, days_since_purchase, region, plan }) {
  ...
  const returnWindowDays = Number.isInteger(v?.return_window_days) ? v.return_window_days : -1;
  const returnType = typeof v?.return_type === "string" ? v.return_type : "";
  const returnSupported = Boolean(v && returnWindowDays > 0 && returnType !== "none");
  const rulebookResult = evaluateReturnPolicyRulebook({
    ...
    within_window: Boolean(returnSupported && days_since_purchase <= returnWindowDays),
    days_since_purchase,
