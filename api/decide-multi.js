...
export default async function handler(req, res) {
  ...
  if (!rateLimitResult.allowed) {
    ...
    await persistLog("decide_multi_request", { request_id, event: "rate_limit_exceeded", ip: clientIp, ua });
    return;
  }
  ...
  try {
    ...
    let stem = String(body.stem || "").trim();
    let options = Array.isArray(body.options) ? body.options.map((item) => String(item || "").trim()).filter(Boolean) : [];

    // Fallback for callers that send one raw pipe-delimited question string.
    if ((!stem || options.length < 2) && body.question) {
      const parsed = parseMultiQuestion(body.question);
