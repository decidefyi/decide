...
export default async function handler(req, res) {
  ...
  try {
    // In Next.js (and Vercel functions) the request body is already
    // parsed if Content-Type is application/json.
    const { question } = req.body || {};
    const q = typeof question === 'string' ? question.trim() : '';

    // Reject very short or empty questions to mirror the client-side
    // behaviour.  Returning an 'unclear' response keeps the UI
    // consistent.
    if (!q || q.length < 3) {
    ...
    // Build the prompt exactly as used on the client.  This ensures
    // consistent behaviour between server and client implementations.
